...
@dataclass(frozen=True)
class VaccineProgramSeasonRecord:
    id: int
    program: str
    season_name: str
    is_active: bool
...
ALLOWED_VACCINE_PROGRAMS = {"influenza", "covid"}
...
def list_vaccine_program_seasons(
    connection: sqlite3.Connection,
    program: str,
) -> list[VaccineProgramSeasonRecord]:
    normalized_program = _validate_vaccine_program(program)
    rows = connection.execute(
        """
        SELECT id, program, season_name, is_active, daily_cap,
               schedule_json, age_groups_json, created_at, updated_at
        FROM vaccine_program_seasons
        WHERE program = ?
        ORDER BY is_active DESC, season_name DESC, id DESC
        """,
        (normalized_program,),
    )
    return [_vaccine_program_season_from_row(row) for row in rows]


def get_vaccine_program_season(
    connection: sqlite3.Connection,
    season_id: int,
) -> VaccineProgramSeasonRecord | None:
    row = connection.execute(
        """
        SELECT id, program, season_name, is_active, daily_cap,
               schedule_json, age_groups_json, created_at, updated_at
    ...
    return _vaccine_program_season_from_row(row) if row is not None else None


def get_active_vaccine_program_season(
    connection: sqlite3.Connection,
    program: str,
) -> VaccineProgramSeasonRecord | None:
    normalized_program = _validate_vaccine_program(program)
...
def create_vaccine_program_season(
    connection: sqlite3.Connection,
    *,
    program: str,
    season_name: str,
    ...
    age_groups: list[dict[str, object]] | None = None,
    ...
    normalized_cap = _validate_vaccine_daily_cap(daily_cap)
    normalized_schedule = dict(schedule or {})
    normalized_schedule["season_name"] = normalized_name
    normalized_schedule["program_enabled"] = bool(is_active)
    normalized_schedule["daily_cap"] = normalized_cap
    normalized_groups = _normalize_vaccine_age_groups(
        age_groups or [], normalized_program
    )
    if is_active:
        connection.execute(
            "UPDATE vaccine_program_seasons SET is_active = 0, updated_at = CURRENT_TIMESTAMP WHERE program = ?",
            (normalized_program,),
        )
    cursor = connection.execute(
        """
        ...
        (
            ...
            int(is_active),
            normalized_cap,
            json.dumps(normalized_schedule, ensure_ascii=False, indent=2),
            json.dumps(normalized_groups, ensure_ascii=False, indent=2),
        ),
    )
    connection.commit()
    created = get_vaccine_program_season(connection, cursor.lastrowid)
    if created is None:
        raise RuntimeError("Failed to create vaccine season.")
    return created


def update_vaccine_program_season(
    connection: sqlite3.Connection,
    season_id: int,
    *,
    season_name: str,
    ...
    current = get_vaccine_program_season(connection, season_id)
    if current is None:
        return None
    normalized_name = season_name.strip()
    if not normalized_name:
    ...
    normalized_schedule["program_enabled"] = bool(is_active)
    normalized_schedule["daily_cap"] = normalized_cap
    normalized_groups = _normalize_vaccine_age_groups(age_groups, current.program)
    if is_active:
        connection.execute(
            """
            UPDATE vaccine_program_seasons
            SET is_active = 0, updated_at = CURRENT_TIMESTAMP
            WHERE program = ? AND id <> ?
            """,
            (current.program, season_id),
        )
    connection.execute(
        """
        UPDATE vaccine_program_seasons
    ...
    connection.commit()
    return get_vaccine_program_season(connection, season_id)


def duplicate_vaccine_program_season(
    connection: sqlite3.Connection,
    season_id: int,
    *,
    ...
    current = get_vaccine_program_season(connection, season_id)
    if current is None:
        raise ValueError("Vaccine season was not found.")
    next_name = (season_name or _next_vaccine_season_name(current.season_name)).strip()
    schedule = _shift_vaccine_dates(current.schedule, years=1)
    age_groups = _shift_vaccine_dates(current.age_groups, years=1)
    return create_vaccine_program_season(
        connection,
        program=current.program,
        season_name=next_name,
...
def delete_vaccine_program_season(
    connection: sqlite3.Connection,
    season_id: int,
) -> bool:
    cursor = connection.execute(
        "DELETE FROM vaccine_program_seasons WHERE id = ?",
        (season_id,),
    )
    connection.commit()
    return cursor.rowcount > 0
...
def sync_active_vaccine_seasons_to_settings(connection: sqlite3.Connection) -> None:
    ...
    try:
        ...
        schedules = {}
    try:
        all_age_groups = json.loads(settings.get("vaccine_age_groups_json", "[]"))
    except (TypeError, json.JSONDecodeError):
        all_age_groups = []
    if not isinstance(schedules, dict):
        schedules = {}
    if not isinstance(all_age_groups, list):
        all_age_groups = []
    ...
    retained_groups = [
        ...
        or str(group.get("vaccine", "")).strip().lower()
        not in ALLOWED_VACCINE_PROGRAMS
    ]
    caps: dict[str, int] = {}
    for program in ("influenza", "covid"):
        active = get_active_vaccine_program_season(connection, program)
        if active is None:
            existing = schedules.get(program)
            section = dict(existing) if isinstance(existing, dict) else {}
            section["program_enabled"] = False
            schedules[program] = section
            continue
        section = dict(active.schedule)
        section["season_name"] = active.season_name
        section["program_enabled"] = True
        section["daily_cap"] = active.daily_cap
        schedules[program] = section
        retained_groups.extend(active.age_groups)
        caps[program] = active.daily_cap

    values = {
        "vaccine_schedule_rules_json": json.dumps(
            schedules, ensure_ascii=False, indent=2
        ),
        "vaccine_age_groups_json": json.dumps(
            retained_groups, ensure_ascii=False, indent=2
        ),
    }
    if "influenza" in caps:
        values["vaccine_influenza_daily_cap"] = str(caps["influenza"])
    if "covid" in caps:
        values["vaccine_covid_daily_cap"] = str(caps["covid"])
...
def _vaccine_program_season_from_row(
    row: sqlite3.Row | tuple,
) -> VaccineProgramSeasonRecord:
    try:
        schedule = json.loads(row[5] or "{}")
    except (TypeError, json.JSONDecodeError):
        schedule = {}
    try:
        age_groups = json.loads(row[6] or "[]")
    except (TypeError, json.JSONDecodeError):
        age_groups = []
    ...
    if not isinstance(age_groups, list):
        age_groups = []
    return VaccineProgramSeasonRecord(
        id=int(row[0]),
        program=str(row[1]),
        ...
        daily_cap=int(row[4]),
        schedule=schedule,
        age_groups=[group for group in age_groups if isinstance(group, dict)],
        created_at=str(row[7]),
        updated_at=str(row[8]),
    )


def _validate_vaccine_program(program: str) -> str:
    normalized = str(program or "").strip().lower()
    if normalized not in ALLOWED_VACCINE_PROGRAMS:
        raise ValueError(f"Unsupported vaccine program: {program}")
    return normalized


def _validate_vaccine_daily_cap(value: int) -> int:
    try:
        cap = int(value)
    except (TypeError, ValueError) as error:
        raise ValueError("Vaccine daily cap must be a whole number.") from error
    if cap < 0:
        raise ValueError("Vaccine daily cap cannot be negative.")
    return cap


def _normalize_vaccine_age_groups(
    age_groups: list[dict[str, object]],
    program: str,
    ...
    for raw_group in age_groups:
        ...
        group["vaccine"] = program
        normalized.append(group)
    return normalized


def _next_vaccine_season_name(name: str) -> str:
    parts = name.strip().split("-")
    if len(parts) == 2 and all(len(part) == 4 and part.isdigit() for part in parts):
        return f"{int(parts[0]) + 1:04d}-{int(parts[1]) + 1:04d}"
    return f"{name.strip()} next".strip()


def _shift_vaccine_dates(value, *, years: int):
    if isinstance(value, dict):
        return {
            key: _shift_vaccine_dates(nested, years=years)
            for key, nested in value.items()
        }
    if isinstance(value, list):
        return [_shift_vaccine_dates(nested, years=years) for nested in value]
    ...
    for format_string in ("%Y-%m-%d", "%Y%m%d"):
        ...
        return shifted.strftime(format_string)
    return value
