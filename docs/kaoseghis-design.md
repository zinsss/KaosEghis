...
- the EMR patient-note alert is a read-only, in-memory safety surface: it waits five
  seconds after a shared patient-change notification, then checks the memo once
  for the `***` marker without a separate chart polling loop; it displays no memo
